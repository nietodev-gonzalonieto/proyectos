...
int fer_jugada(joc_t *j) {
    ...
    switch(j->torn) {
        // Juega el ordenador
        case 0:   
            // El ordenador elige la posición que maximiza la suma total de las posiciones restantes
            
            for (int i = 0; i < MIDA_TAULER; i++) {
                if (j->tauler[i] == '-') {
                    suma = j->punts[0] + j->punts[1] + (i + 1);
                    if (suma > suma_maxima) {
                        suma_maxima = suma;
                        posicio = i;
                    }
        ...
        case 1:  
            // Solicitar al jugador que elija una posición válida
