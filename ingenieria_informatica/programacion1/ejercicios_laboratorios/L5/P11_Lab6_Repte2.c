/*
 ============================================================================
 Name        : P11_Lab6_Repte2.c
 Author      : GonzaloNieto
 Version     :
 Copyright   : Esto es obra de Gonzalo Nieto
 Description : Lab6_Repte2
 ============================================================================
 */
...
//Abrimos , Comprobamos fichero y guardamos en array los valores
...
//Recorremos la Array y guardamos sensores defectuosos , convertimos a Celsius los NO defectuosos y hacemos sumatori
...
//Printamos las temperaturas en Celcius y comprobamos medias aprovechando el mismo bucle
...
//Calculamos el MIN y el MAX
	max=temperaturas_celcius[0], min=temperaturas_celcius[0];
	for (int i =0; i<contador; i++){
		if (temperaturas_celcius[i] != -999.9f && max < temperaturas_celcius[i]){
			max=temperaturas_celcius[i];
		}
		if(temperaturas_celcius[i] != -999.9f && min > temperaturas_celcius[i]){
			min=temperaturas_celcius[i];
		}
	}
...
//Printamos cuantos sensores estan por encima y por debajo de la media
...
//Printamos cual es la temperatura minima y maxima
